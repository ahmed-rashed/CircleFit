function [Mode_Inf,circ_prop] = FRF_CircleFit(f_local,Receptance_local,ShowInternalDetails)

%Error checking
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% In the case of insufficient number of samples
N_pts = length(f_local); 
if N_pts < 5
    error('Insufficient number of points in the studied frequency range');
end

%Check f_local is equispaced with a positive d_f
d_f=f_local(2)-f_local(1);
n=(f_local(2:end)-f_local(1:end-1))/d_f;
...
% Circle Fitting
[x_center,y_center,Radius] = err_fit_circle(real(Receptance_local),imag(Receptance_local));
circ_prop = struct('x_center',x_center,'y_center',y_center,'Radius',Radius);

%Identify the two points surrounding w_r
chord=sqrt((real(Receptance_local(2:end))-real(Receptance_local(1:end-1))).^2+(imag(Receptance_local(2:end))-imag(Receptance_local(1:end-1))).^2);
[chord_max,index]=max(chord);
f_before=f_local(index);
f_after=f_local(index+1);
...
beta = atan2(imag(Receptance_local)-y_center,real(Receptance_local)-x_center);  %Returns the radial angles wrt the x-axis. Angles in the range [-pi~pi]
...
%Calculate eta_r (modal damping)
    ...
    w_before = 2*pi*f_local(flag_before);
    w_after = 2*pi*f_local(flag_after);
...
% Output
Mode_Inf = struct('f_r',f_r,'eta_r',eta_r,'A_r',A_r,'B_r',B_r);

if ShowInternalDetails
    %For manual Display Only
    f_before
    f_after
    f_interp=f_local(index-1:index+2).'
    ...
    Receptance_gen_local=A_r./(complex(w_r^2-(2*pi*f_local).^2,eta_r*w_r^2)) + B_r;
    CircleFitInternalDetails(f_local,Receptance_local,Receptance_gen_local,circ_prop,beta_r,w_before_mat/2/pi,w_after_mat/2/pi,eta_r_mat);
end